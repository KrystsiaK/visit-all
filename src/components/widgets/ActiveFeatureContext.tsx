...
const ActiveFeatureContext = createContext<WidgetContextType | undefined>(undefined);

export function ActiveFeatureProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ActiveFeatureContext.Provider value={{
...
/** @deprecated Use `ActiveFeatureProvider` — `WidgetProvider` name is reserved for the framework chrome context. */
export const WidgetProvider = ActiveFeatureProvider;

export function useActiveFeature() {
  const context = useContext(ActiveFeatureContext);
  if (context === undefined) {
    throw new Error("useActiveFeature must be used within an ActiveFeatureProvider");
  }
  return context;
}

/** @deprecated Use `useActiveFeature` — `useWidgetContext` name is reserved for the framework chrome context hook. */
export const useWidgetContext = useActiveFeature;
